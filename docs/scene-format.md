...
Root documents contain `version: 1`, `canvas`, `layers` and/or `include`, optional `assets`, `metadata`, and `verification: {rules: [...]}`. Fragments contain only `include`, `assets` and `layers`. JSON and YAML can be mixed. There can be at most 128 included documents and 256 total layers.
...
Layer effects currently support `{type: blur, radius: 8}`. Rendering order is geometry/children → effects → palette tiles → raster operations → layer mask → opacity → sibling blending. Adjustment layers apply scoped operations to the existing composite, then mix the result by their mask and opacity.
...
`replaceTile` replaces a tile with exactly matching bounds or appends a new one. Transparent tile pixels composite over underlying layer content. This is an addressable palette representation, not yet an adaptive quadtree or automatic promotion system.
