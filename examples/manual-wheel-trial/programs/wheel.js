...
if (pass === 'tyre') {
  const height = (x, y) => {
    const { d } = wheel(x, y);
    return 48 * Math.sqrt(Math.max(0, 1 - ((d - 2610) / 95) ** 2));
  };
  return art.raster((xx, yy) => {
    const x = xx / S,
      y = yy / S,
    ...
    const small = (art.noise(x * 1.9, y * 1.9) - 0.5) * 2;
    const cloud = (art.fbm(x / 65, y / 48, 4) - 0.5) * 4;
    const dust = clamp((art.fbm(x / 3, y / 3, 3, 83) - 0.62) * 18, 0, 3);
    let v = small + cloud + dust;
    if (d < 2540) v += 2 * Math.sin(a * 1900) * Math.sin(d * 1.25);
    const n = art.normal(height, x, y);
    const result = art.light(n, [40 + v, 47 + v, 51 + v, edge * 255], {
      light: [-0.65, -0.35, 1],
      ambient: 0.24,
      diffuse: 0.85,
      specular: 0.055,
    ...
    const bead = 1 - 0.32 * gauss(d, 2529, 11);
    return [result[0] * bead, result[1] * bead, result[2] * bead, result[3]];
  });
}
if (pass === 'tread') {
  function block(x, y) {
    const { d, a } = wheel(x, y),
      index = Math.round((a + 3.22) / 0.026),
      centre = -3.22 + index * 0.026;
    const u = d - 2684,
      v = (a - centre) * 2684;
    const qx = Math.abs(u - 14) - 24,
      qy = Math.abs(v) - 15;
    const distance =
      Math.hypot(Math.max(qx, 0), Math.max(qy, 0)) + Math.min(Math.max(qx, qy), 0) - 4;
    const slit =
      gauss(v + 5 - u * 0.1, 0, 0.8) * art.smoothstep(-2, 3, u) * (1 - art.smoothstep(30, 36, u));
    return { u, v, distance, slit, index };
  }
  function height(x, y) {
    const b = block(x, y);
    return (
      14 * art.smoothstep(1, -5, b.distance) +
      3 * gauss(b.u, 8, 25) * gauss(b.v, 0, 17) -
      1.5 * b.slit
    );
  }
  return art.raster((xx, yy) => {
    const x = xx / S,
      y = yy / S,
      b = block(x, y);
    if (b.distance > 0.7 || b.index < 0 || b.index > 34) return [0, 0, 0, 0];
    const n = art.normal(height, x, y),
      grain = (art.noise(x * 1.3, y * 1.3) - 0.5) * 2;
    const light = art.light(
      n,
      [46 + grain, 54 + grain, 58 + grain, clamp(0.7 - b.distance) * 255],
      { light: [-0.65, -0.5, 1], ambient: 0.2, diffuse: 0.85, specular: 0.06, shininess: 22 },
    );
    const crack = 1 - b.slit * 0.28;
    return [light[0] * crack, light[1] * crack, light[2] * crack, light[3]];
  });
}
if (pass === 'rim') {
  return art.raster((xx, yy) => {
    ...
    if (d < 2460) v = 32 + gauss(d, 2453, 3) * 28 + grain;
      ...
      v = 153 + gauss(d, 2489, 17) * 52 + gauss(d, 2464, 2) * 34 - 3 * Math.sin(d * 2.2) + grain;
    ...
    if (detail && d >= 2460 && d < 2508)
      v +=
        7 * gauss(d, 2503, 1) -
        3 * gauss(d, 2472, 0.6) +
        3 * (art.noise(x * 1.6, y * 1.6) - 0.5) +
        15 * (art.fbm(a * 1200, d / 3, 4, 97) - 0.5) -
        16 * gauss(y, 1050, 210);
...
if (pass === 'wear') {
  ...
  for (let k = 0; k < 180; k++) {
    ...
    art.brush(
      ...
      {
        radius: (0.3 + art.random() * 0.7) * S,
        colour: '#a6a297',
        hardness: 0.5,
        opacity: 0.04 + art.random() * 0.06,
        spacing: 0.5,
      },
    );
  }
  ...
  // Hand-placed broader scuffs interrupt the mathematical regularity of the material.
  for (const [a, r, length] of [
    [-2.83, 2630, 0.018],
    [-2.96, 2562, 0.009],
    [-2.64, 2585, 0.012],
  ]) {
